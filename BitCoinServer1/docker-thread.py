...
def getBinary(val):
    ...
    while val > 0:
        ...
        val = int(val / 2)
    print("...Server1 produce number:",val_raw," and calculate its binary number is:",binary_list,"At ",time.ctime())
    ...
    def run(self):
        while self.listen_count < 10:
            ...
            if Addr[0] == '172.17.0.3':
                print(f"---Server1 get message from Server2:{from_msg.decode('gbk')}")
                self.listen_count += 1
                socket.close()
            else:
                print(f"+++Server1 get message from Client1:{from_msg.decode('gbk')}")
                socket.close()
        self.socket.close()
